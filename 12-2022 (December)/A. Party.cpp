...
void bfs(ll node, vl &level, vector<bool> &visited, vector<vl> &graph) {
    visited[node] = true;
    queue<ll> q;
    q.push(node);
...
void solve() {
    ...
    // fl(i,0,n){
    //     cout<<adj[i]<<nl; // 1 2 3   5 6 7   9 10 11
    // }
    // cout<<roots<<nl; // 0 4 8
    ...
    for (auto i : roots) {
        level[i] = 1;
        bfs(i, level, visited, adj);
    }
    cout << max(mxlvl, (ll)1) << nl;
}
