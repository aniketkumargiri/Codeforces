...
void solve() {
    ...
    cin >> a;
    prefix[0] = a[0];
    fl(i, 1, n) { prefix[i] = prefix[i - 1] + a[i]; }
    ll m;
    cin >> m;
    fl(i, 0, m) {
        ll q;
        cin >> q;
        if (binary_search(all(prefix), q)) {
            cout << upper_bound(all(prefix), q) - prefix.begin() << nl;
        } else {
            cout << upper_bound(all(prefix), q) - prefix.begin() + 1 << nl;
        }
    }
}
