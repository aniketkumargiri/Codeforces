...
/*********************Helper********************/
ll dp[101][3];
ll minRestDays(vl &a, ll n, ll i, ll done) {
    if (i >= n) return 0;
    if (dp[i][done] != -1) return dp[i][done];

    if (a[i] == 0)
        dp[i][done] = minRestDays(a, n, i + 1, 0) + 1;
    else if (a[i] == 1) {
        if (done == a[i])
            dp[i][done] = minRestDays(a, n, i + 1, 0) + 1;
        else
            dp[i][done] = minRestDays(a, n, i + 1, 1);
    } else if (a[i] == 2) {
        if (done == a[i])
            dp[i][done] = minRestDays(a, n, i + 1, 0) + 1;
        else
            dp[i][done] = minRestDays(a, n, i + 1, 2);
    } else if (a[i] == 3) {
        ll x = done == 1 ? maxi : minRestDays(a, n, i + 1, 1);
        ll y = done == 2 ? maxi : minRestDays(a, n, i + 1, 2);
        dp[i][done] = min(x, y);
    }
    return dp[i][done];
}
