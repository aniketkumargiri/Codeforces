...
void solve() {
    ...
    m2 += t[3];
    m2 += t[4];
    ll s1 = stoi(h1), s2 = stoi(m1), t1 = stoi(h2), t2 = stoi(m2);
    ll time1 = s1 * 60 + s2, time2 = t1 * 60 + t2;
    // cout<<time1<<sp<<time2<<nl;
    ll less = time1 - time2;
    if (less < 0) {
        less += 1440;
    }
    string fi = to_string(less / 60), se = to_string(less % 60);
    if (fi.size() == 1) fi = '0' + fi;
    if (se.size() == 1) se = '0' + se;
    cout << fi << ":" << se << nl;
}
