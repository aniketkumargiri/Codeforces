...
#define arrMax(v) max_element(all(v))
#define arrMin(v) min_element(all(v))
...
bool isPowerOfTwo(ll n) { return setbits(n) == 1; }
...
void rotateArr(vl &a, ll k);
vl nextPermutation(vl &a);

/*********************Helper********************/

/*********************Solve Code********************/
void solve() {
    // Add your code here (***Remember Edge Cases***)
    string s;
    cin >> s;
    vector<string> v = {"01:10", "02:20", "03:30", "04:40", "05:50", "10:01",
                        "11:11", "12:21", "13:31", "14:41", "15:51", "20:02",
                        "21:12", "22:22", "23:32", "00:00"};
    ll n = v.size();
    fl(i, 0, n) {
        if (v[i] == s) {
            cout << v[(i + 1) % n] << nl;
    ...
    string r = h;
    reverse(all(r));
    ll ht = stoi(h), mt = stoi(m), rn = stoi(r);
    // cout<<ht<<sp<<mt<<sp<<rn<<nl;
    if (ht <= 5 || (ht >= 10 && ht <= 15) || ht >= 20) {
        if (ht == 5 && mt >= 50)
            cout << "10:01\n";
        else if (ht == 15 && mt >= 51)
            cout << "20:02\n";
...
/************Next Permutation**************/
vl nextPermutation(vl &nums) {
    ll n = nums.size(), i = n - 2;
