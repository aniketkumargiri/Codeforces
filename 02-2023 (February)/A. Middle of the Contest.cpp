...
void solve() {
    ...
    t2 += t[3];
    t2 += t[4];
    ll h2 = stoi(t1), m2 = stoi(t2);
    // cout<<h2<<sp<<m2<<nl;

    ll d1 = h1 * 60 + m1, d2 = h2 * 60 + m2;
    // cout<<d1<<sp<<d2<<nl;
    ll dif = (d2 - d1) / 2.0;
    if (dif < 0) dif += 1440;
    // cout<<dif<<nl;
    ll avgh = dif / 60, avgm = dif % 60;
    ll ans1 = h1 + avgh, ans2 = m1 + avgm;
    if (ans2 >= 60) {
        ans1++;
        ans2 %= 60;
    }
    string str1 = to_string(ans1), str2 = to_string(ans2);
    if (str1.size() == 1) {
        str1 = '0' + str1;
    }
    if (str2.size() == 1) {
