...
void solve() {
    ...
    fl(i, 0, n - 1) {
        vl tmp;
        fl(j, 0, n) { tmp.pu(a[j][i]); }
        freq.pu(tmp);
    }
    // for(auto i:freq) cout<<i<<nl;
    vl ans;
    ...
    for (auto arr : freq) {
        ...
        ll store = 0;
        for (auto i : mp) {
            if (i.second > n / 2 && !vis[i.first]) {
                ans.pu(i.first);
                vis[i.first] = true;
            } else {
                if (!vis[i.first]) store = i.first;
            }
        }
        cnt++;
        if (n % 2 != 0 && cnt == (n - 1) / 2 && !vis[store]) {
            ans.pu(store);
            vis[store] = true;
        } else if (n % 2 == 0 && cnt == n / 2 - 1 && !vis[store]) {
            ans.pu(store);
            vis[store] = true;
        } else if (n % 2 == 0 && cnt == n / 2 && !vis[store]) {
            ans.pu(store);
            vis[store] = true;
        }
    }
