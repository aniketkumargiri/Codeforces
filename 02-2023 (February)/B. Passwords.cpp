...
void solve() {
    ...
    sort(all(v), cmp);
    // cout<<v<<nl;
    ll cnt1 = 0, cnt2 = 0, i = 0;
    while (s.size() > v[i].size()) {
        i++;
        cnt1++;
    }
    while (s.size() == v[i].size()) {
    ...
    // cout<<cnt<<nl;
    if (k >= n) {
        cout << cnt1 + 1 << sp << cnt1 + cnt2 << nl;
        rt;
    }
    ll cd1 = ceil_div(cnt1 + cnt2, k) - 1;
    ll cd2 = ceil_div(cnt1 + 1, k) - 1;
    cout << cd2 * 5 + cnt1 + 1 << sp << cd1 * 5 + cnt1 + cnt2 << nl;
}
