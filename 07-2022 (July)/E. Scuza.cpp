...
void ani() {
    ...
    vl max_prefix(n), sum_prefix(n);
    max_prefix[0] = a[0];
    fl(i, 1, n) { max_prefix[i] = max(max_prefix[i - 1], a[i]); }
    sum_prefix[0] = a[0];
    fl(i, 1, n) { sum_prefix[i] = sum_prefix[i - 1] + a[i]; }

    fl(i, 0, k) {
        ll idx = upper_bound(max_prefix.begin(), max_prefix.end(), q[i]) -
                 max_prefix.begin();
        if (idx == 0) {
            cout << 0 << sp;
        } else {
            cout << sum_prefix[idx - 1] << sp;
        }
    }
    cout << nl;
}
