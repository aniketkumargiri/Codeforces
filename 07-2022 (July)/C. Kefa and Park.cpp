...
void dfs(ll parent){
    visited[parent]=true;
    if(cats[parent]>m) return;
    if(cats[parent]<=m && isLeaf(parent)) ans++;

    for(auto child:adj[parent]){
        if(!visited[child]){
            if(cats[child]) cats[child]+=cats[parent];
            dfs(child);
        }
    }
}
...
void solve() {
    ...
    cin>>n>>m; fl(i,0,n) cin>>cats[i];
    fl(i,0,n-1){
        ll u,v;cin>>u>>v;u--,v--;
        adj[u].pu(v);adj[v].pu(u);
    }
    // fl(i,0,n) cout<<adj[i]<<nl;
    dfs(0);
    cout<<ans<<nl;
}
...
bool isSubsetSum(vl arr, int sum) {
    ...
    for (ll j = 0; j < sum + 1; j++) { dp[0][j] = false; }
