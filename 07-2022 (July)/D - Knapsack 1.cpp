...
/*********************Helper********************/
ll knapsack(ll n, ll w, vl wt, vl v, vvl &dp) {
    if (n == 0 || w == 0) {
        return 0;
    }
    ...
    if (wt[n - 1] <= w) {
        dp[n][w] = max(v[n - 1] + knapsack(n - 1, w - wt[n - 1], wt, v, dp),
                       knapsack(n - 1, w, wt, v, dp));
    } else {
        dp[n][w] = knapsack(n - 1, w, wt, v, dp);
    }
    return dp[n][w];
}
...
void ani() {
    ...
    vl wt(n), v(n);
    fl(i, 0, n) { cin >> wt[i] >> v[i]; }

    // vvl dp(n+1, vl(w+1, -1));
    // ll ans=knapsack(n, w, wt, v, dp);
    // cout<<ans<<nl;
    ...
    fl(i, 1, n + 1) {
        fl(j, 1, w + 1) {
            if (wt[i - 1] <= j) {
                dp[i][j] =
                    max(v[i - 1] + dp[i - 1][j - wt[i - 1]], dp[i - 1][j]);
            } else {
                dp[i][j] = dp[i - 1][j];
            }
        }
