...
void solve() {
    ...
    unordered_map<string, ll> mp1, mp2, mp3;
    fl(i, 0, n) {
        string s1;
        cin >> s1;
        v1.pu(s1);
    ...
    for (auto i : v1) {
        if (mp2.find(i) == mp2.end() && mp3.find(i) == mp3.end()) {
            ans1 += 3;
        } else if (mp2.find(i) == mp2.end() && mp3.find(i) != mp3.end()) {
            ans1 += 1;
        } else if (mp2.find(i) != mp2.end() && mp3.find(i) == mp3.end()) {
            ans1 += 1;
        } else if (mp2.find(i) != mp2.end() && mp3.find(i) != mp3.end()) {
    ...
    for (auto j : v2) {
        if (mp1.find(j) == mp1.end() && mp3.find(j) == mp2.end()) {
            ans2 += 3;
        } else if (mp1.find(j) == mp1.end() && mp3.find(j) != mp2.end()) {
            ans2 += 1;
        } else if (mp1.find(j) != mp1.end() && mp3.find(j) == mp2.end()) {
            ans2 += 1;
        } else if (mp1.find(j) != mp1.end() && mp3.find(j) != mp2.end()) {
    ...
    for (auto k : v3) {
        if (mp1.find(k) == mp1.end() && mp2.find(k) == mp2.end()) {
            ans3 += 3;
        } else if (mp1.find(k) == mp1.end() && mp2.find(k) != mp2.end()) {
            ans3 += 1;
        } else if (mp1.find(k) != mp1.end() && mp2.find(k) == mp2.end()) {
            ans3 += 1;
        } else if (mp1.find(k) != mp1.end() && mp2.find(k) != mp2.end()) {
