...
void solve() {
    ...
    string s, t;
    cin >> s >> t;
    ll n1 = s.size(), n2 = t.size(), c1 = 0, c2 = 0, cnt = 0;
    for (ll i = 0; i < n && i < n1 * 1LL * n2; i++) {
        char ch1 = s[i % n1], ch2 = t[i % n2];
        if (ch1 == ch2) {
            cnt++;
        } else {
            ll tmp = (n - i + n1 * 1LL * n2 - 1) / (n1 * 1LL * n2);
            if ((ch1 == 'R' && ch2 == 'S') || (ch1 == 'S' && ch2 == 'P') ||
                (ch1 == 'P' && ch2 == 'R')) {
                c1 += tmp;
            } else {
                c2 += tmp;
